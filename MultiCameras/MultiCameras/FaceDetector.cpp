...
void FaceDetector::Detect(IplImage* pImg) {
	...
	cout<<"Time elapsed for _FaceDetection:"<<totaltime<<endl;


	#pragma omp parallel for  
	for (int i = 0;i<after_mer;i++)
	{			   
		//cout<<after_mer<<" "<<draw_point.at<float>(i,5)<<endl;
		//double area = calcArea2(draw_point.at<float>(i,1), draw_point.at<float>(i,3), draw_point.at<float>(i,0), draw_point.at<float>(i,2));
		if (draw_point.at<float>(i,5)>10 /*&& area < FACE_AREA_MAX && area > FACE_AREA_MIN*/)
		{
			...
			cout<<draw_point.at<float>(i,1)<<" "<<draw_point.at<float>(i,0)<<" "<<draw_point.at<float>(i,3)<<" "<<draw_point.at<float>(i,2)<<endl;
			detected_faces++;
		}
	}
...
Mat FaceDetector::_FaceDetection(IplImage* pImg, int &after_mer) {
	...
	finish = clock();
	double totaltime=(double)(finish-start)/CLOCKS_PER_SEC;
	cout<<"Time elapsed for create pym:"<<totaltime<<endl;
	/*****************************************************检测过程**********************************************************/
	...
	cout<<"Time elapsed for create detect:"<<totaltime<<endl;
	/************************************************合并检测窗口*********************************************************/
